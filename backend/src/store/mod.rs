...
impl Store {
    pub async fn init(app_id: &str) -> Result<Self, StoreError> {
...
fn app_data_dir(app_id: &str) -> Result<PathBuf, StoreError> {
    let base = dirs::data_local_dir()
        .or_else(dirs::data_dir)
        .ok_or_else(|| StoreError::InitError("cannot resolve application data directory".into()))?;
